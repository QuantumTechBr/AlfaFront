...
export default function DashboardDiagnosticaView() {
  ...
  // + zona via escola. Usado como ddzOptions para alternância entre zonas.
  // Para SUPERADMIN/sem-funcao, mostra todas as zonas do sistema.
  ...
  const preencheGraficos = useCallback(
    async (_filters) => {
      ...
      if (fullFilters.ddz.length == 0) {
        await Promise.all([
          dashboardsMethods.getDashboardAvaliacaoDiagnosticoRede2(fullFilters).then((response) => {
        ...
        await Promise.all([
          dashboardsMethods.getDashboardAvaliacaoDiagnostico2(fullFilters).then((response) => {
            const result = Object.assign({}, response.data);
            prepareData(result);
          }).catch((error) => {
            if (error.response?.data?.message) {
  ...
  useEffect(() => {
    if (user && contextReady.value) {
      ...
      if (user?.funcao_usuario?.length > 0) {
        ...
        // funcao_usuario). Inclui zona direta (ASSESSOR DDZ, COORDENADOR DE
        // GESTAO) e zona via escola (DIRETOR/PROFESSOR/etc).
        const _zonasMap = new Map();
        for (const fu of user.funcao_usuario) {
          const zonaDireta = fu?.zona;
          const zonaViaEscola = fu?.escola?.zona;
        ...
        >
          <Grid xs={12} md>
            ...
            <Typography variant="h3">Dashboard: Avaliação Diagnóstica</Typography>
              <InstructionButton youtubeLink={"https://www.youtube.com/embed/fxgNcBmSqYQ?si=C-YQ95rK6y6NlODS&cc_load_policy=1"} />
            </Box>
          </Grid>
        </Stack>
              ...
              <Grid container marginX={0} spacing={3} marginTop={3} width="100%">
                <Grid xs={12} md={4} lg={4}>
                  <NumeroComponent
                    ...
                    title="Estudantes"
                    subtitle="Entrada"
                    total={dados.total_alunos?.entrada ?? 0}
                  />
                </Grid>
                <Grid xs={12} md={4} lg={4}>
                  <NumeroComponent
