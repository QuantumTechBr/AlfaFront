...
//
import AvaliacaoDiagnosticoTableRow from './avaliacao-diagnostico-table-row';
...
export default function AvaliacaoDiagnosticoListView() {
  ...
  const buscarAvaliacoes = useCallback(async (pagina = 0, linhasPorPagina = 25, prevList = [], filtros = filters) => {
    if (contextReady.value && anosLetivos.length && turmas.length) {
      ...
      await avaliacaoMethods
        .getListIdTurmaAvaliacaoAluno(_filtersToSend)
        .then((response) => {
        ...
        .catch((error) => {
          setErrorMsg('Erro de comunicação com a API de Avaliação');
          buscando.onFalse()
          console.error(error);
        });
      buscando.onFalse();
  ...
  const handleEditRow = useCallback(
    (id) => {
      router.push(paths.dashboard.avaliacao.edit_diagnostico(id));
    },
    [router]
  );
  ...
  const handleDeleteRow = useCallback(
    (id, periodo) => {
      ...
      avaliacaoMethods
        .deleteAvaliacaoAlunoByFilter({ tipo: TipoVersaoAvaliacao.DIAGNOSTICA, turmaId: id, periodo: periodo })
        .then(
  ...
  const uploadAvaliacoes = async () => {
    try {
      ...
      const response = await avaliacaoMethods.importFileDiagnostico(formData);
            ...
            <Scrollbar>
              ...
              {contextReady.value && tabelaPreparada.value && (
                <Table size="small" sx={{ minWidth: 960 }}>
                  ...
                  <TableBody>
                    {dataInPage.map((row) => (
                      <AvaliacaoDiagnosticoTableRow
                        key={`AvaliacaoDiagnosticoTableRow_${row.id}_${row.periodo}`}
                        row={row}
