...
const TABLE_HEAD = [
  ...
  { id: 'nome', label: 'Profissional', width: 300 },
  { id: 'funcao', label: 'Função', width: 200 },
  { id: 'escola', label: 'Escola', width: 300 },
  { id: 'zona', label: 'Zona', width: 300 },
  { id: '', width: 88 },
  { id: '', width: 88 },
];
...
export default function ProfissionalListView() {
  ...
  useEffect(() => {
    profissionalMethods.getAllProfissionais().then(profissionais => {
      for (var i = 0; i < profissionais.data.length; i++) {
        profissionais.data[i].status = 'ativo';
      }
...
function applyFilter({ inputData, comparator, filters }) {
  ...
  if (nome) {
    inputData = inputData.filter(
      (profissional) => profissional.nome.toLowerCase().indexOf(nome.toLowerCase()) !== -1
    );
  }
